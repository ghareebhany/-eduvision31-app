...
class _RedeemCodeScreenState extends ConsumerState<RedeemCodeScreen> {
  ...
  /// بعد نجاح الكود: حدّث الحالة محلياً مؤقتاً، ثم اعتمد على الخادم كحكم نهائي.
  /// إن فشل تثبيت التحديث من الخادم نتراجع عن التحديث التفاؤلي بدل تجميد «مسجّل» في الذاكرة.
  Future<void> _onSuccess(Map<String, dynamic> result) async {
    ...
    // 3) الخادم هو مصدر الحقيقة: إن فشل الجلب تراجع عن التحديث التفاؤلي
    final notifier = ref.read(bundlesProvider.notifier);
    final confirmed = await notifier.fetch(refresh: true);
    if (!confirmed) {
      notifier.revertOptimistic();
      // اعتبر الحالة قديمة حتى تُعاد من الخادم عند عودة شاشة الباقات
      notifier.markStale();
    }
  }
